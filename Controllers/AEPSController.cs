...
namespace Grofinhub.Controllers
{
    public class AEPSController : Controller
    {
        ...
        public IActionResult Get2FAStatus()
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                var result = new
                {
                    ...
                    FingerprintData = dt.Rows[0]["FingerprintData"].ToString()
                };
        ...
        public JsonResult DailyAEPSAuth(AEPS2FARagistrationModel p)
        {
            try
            {
                if (Convert.ToString(HttpContext.Session.GetString("Role")) != "2")
                ...
                {
                    //save the status into the database
                    var res = db.UpdateAeps2FARegistrationStatus(userid, 1, p.data, "DailyAuth");
                }
                return Json(response.Content);
            }
            catch
